...
class FdLibm {
    ...
    private static final float INFINITY_F = Float.POSITIVE_INFINITY;
    ...
    /**
     * Return the low-order 16 bits of the float argument as a short.
     */
    private static short __LO(float x) {
        int transducer = Float.floatToRawIntBits(x);
        return (short)transducer;
    }
    ...
    /**
     * Return a float with its low-order bits of the second argument
     * and the high-order bits of the first argument..
     */
    private static float __LO(float x, short low) {
        int transX = Float.floatToRawIntBits(x);
        return Float.intBitsToFloat((transX & 0xFFFF_0000) |
                                       (low    & 0x0000_FFFF));
    }
    ...
    /**
     * Return the high-order 16 bits of the float argument as a short.
     */
    private static short __HI(float x) {
        int transducer = Float.floatToRawIntBits(x);
        return (short)(transducer >> 16);
    }
    ...
    /**
     * Return a float with its high-order bits of the second argument
     * and the low-order bits of the first argument..
     */
    private static float __HI(float x, short high) {
        int transX = Float.floatToRawIntBits(x);
        return Float.intBitsToFloat((transX & 0x0000_FFFF) |
                                       ( ((int)high)) << 16 );
    }
    ...
    public static class Cbrt {
        ...
        public static strictfp float compute(float x) {
            return (float)compute((double)x);
        }
    }
    ...
    public static class Hypot {
        ...
        public static strictfp float compute(float x, float y) {
            return (float)compute((double)x, (double)y);
        }
    }
    ...
    public static class Pow {
        ...
        public static strictfp float compute(final float x, final float y) {
            return (float)compute((double)x, (double)y);
        }

        public static strictfp double compute(final double x, final int y) {
            double z;
            double r, s, t, u, v, w;
            int i, j, k, n;
            ...
            double p_h, p_l, t1, t2;
            // |y| is huge
            if (y_abs == Integer.MAX_VALUE) { // if |y| > ~2**31
                final double INV_LN2   =  0x1.7154_7652_b82fep0;   //  1.44269504088896338700e+00 = 1/ln2
                final double INV_LN2_H =  0x1.715476p0;            //  1.44269502162933349609e+00 = 24 bits of 1/ln2
                final double INV_LN2_L =  0x1.4ae0_bf85_ddf44p-26; //  1.92596299112661746887e-08 = 1/ln2 tail
        ...
        public static strictfp float compute(final float x, final int y) {
            return (float)compute((double)x, y);
        }
    }
