...
Java_java_lang_ClassLoader_getClassBytes(JNIEnv *env,
                                        jclass cls,
                                        jobject loader,
                                        jstring name)
{
    ...
    char buf[128];
    char *utfName = getUTF(env, name, buf, sizeof(buf));
    if (utfName == NULL) {
        return 0;
    }
    fixClassname(utfName);

    auto buffer = get_cached_class(utfName);
    free(utfName);
    ...
    env->SetByteArrayRegion(ret, 0, buffer.size, buffer.data);
                                        ...
                                        jstring source)
{
    ...
    assert(env->GetDirectBufferCapacity(data) >= (offset + length));

    body = (jbyte *)env->GetDirectBufferAddress(data);
